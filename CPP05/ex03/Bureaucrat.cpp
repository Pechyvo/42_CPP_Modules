...
void Bureaucrat::executeForm(AForm const & form) const {
	try {
		form.execute(*this);
		std::cout << U_GREEN << this->name << GREEN << " executed " << U_GREEN << form.getName() << RESET << std::endl;
	}
	catch (const std::exception& e) {
		std::cout << U_RED << this->name << RED << " couldn't execute " << U_RED << form.getName() << RED << " because " << e.what() << RESET << std::endl;
	}
}
